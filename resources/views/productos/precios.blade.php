...
@section('contenido')
<form method="POST" action="{{ route('productos.precios.update') }}" class="card" style="max-width:820px">
    @csrf
    <div class="card-body">
        <div class="row g-2 align-items-end mb-3">
            <div class="col-sm-5"><label class="form-label" for="apTipo">Aplicar aumento a</label><select class="form-select" id="apTipo"><option value="">Todos los productos</option>@foreach ($tipos as $t)<option value="{{ $t->id }}">{{ $t->nombre }}</option>@endforeach</select></div>
        ...
        <div class="table-responsive"><table class="table align-middle">
            ...
            <tbody>@foreach ($productos as $p)
                <tr data-tipo="{{ $p->tipo_producto_id }}"><td>{{ $p->nombre }}</td><td class="num">{{ pesos($p->precio_actual) }}</td>
    ...
    <div class="card-body border-top d-flex gap-2 justify-content-end"><a class="btn btn-light" href="{{ route('productos.index') }}">Cancelar</a><button class="btn btn-primary">Guardar precios</button></div>
</form>
@endsection
