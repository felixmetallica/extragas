...
<div class="login-wrap">
    <section class="login-art">
        <div class="d-flex align-items-center gap-2"><img src="<?= asset('img/logo.svg') ?>" width="40" height="40" alt=""><span class="fs-4 fw-bold"><?= e($empresa['nombre']) ?></span></div>
        <div>
    ...
    <section class="login-form">
        <form method="POST" action="<?= url('login') ?>">
            <?= csrf_campo() ?>
            <div class="d-md-none d-flex align-items-center gap-2 mb-4"><img src="<?= asset('img/logo.svg') ?>" width="36" height="36" alt=""><span class="fs-4 fw-bold"><?= e($empresa['nombre']) ?></span></div>
            <h1 class="h3 fw-bold mb-1">Ingresar</h1>
            <p class="text-body-secondary mb-4">Accedé con tu usuario del sistema.</p>
            <?php if ($err = error_de('username') ?? error_de('general')): ?><div class="alert alert-danger py-2 small"><?= e($err) ?></div><?php endif ?>
            <div class="mb-3"><label class="form-label" for="username">Usuario</label>
                <div class="input-group"><span class="input-group-text"><i class="bi bi-person"></i></span>
                    <input class="form-control" id="username" name="username" value="<?= e(viejo('username')) ?>" required autofocus autocomplete="username"></div></div>
